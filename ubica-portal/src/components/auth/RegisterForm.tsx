...
const RegisterForm: React.FC<RegisterFormProps> = ({ isEmbedded }) => {
  ...
  const formContent = (
    <div className={isEmbedded ? "max-h-[550px] overflow-y-auto pr-2 custom-scrollbar" : "max-w-md w-full bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700"}>
      <div>
        <h2 className={`text-center font-black text-gray-900 dark:text-white ${isEmbedded ? 'text-4xl mb-2' : 'text-3xl mt-4'}`}>
          {t('auth.register_title')}
        </h2>
        <p className={`text-center text-gray-500 dark:text-gray-400 ${isEmbedded ? 'text-base mb-6' : 'mt-2 text-sm'}`}>
